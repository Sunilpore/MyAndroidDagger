...
public class PostViewModel extends ViewModel {
    ...
    public LiveData<Resource<List<Post>>> observePosts(){
        if(posts == null){
            ...
            posts.setValue(Resource.<List<Post>>loading(null));

        final LiveData<Resource<List<Post>>> source = LiveDataReactiveStreams.fromPublisher(

                //To avoid complexity of code here we directly take a userId from session Manager without observing it
                ...
                .onErrorReturn(new Function<Throwable, List<Post>>() {
                    ...
                    public List<Post> apply(Throwable throwable) throws Exception {
                        LogHelper.showLogError(throwable.getMessage());
                        Post post = new Post();
                        post.setId(-1);
